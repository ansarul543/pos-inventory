...
class ProductAdjutment(QDialog):
    ...
    def loadDataDate(self):
        ...
        if sv=="":
            ...
            result = cur.execute("SELECT proadjust.id,products.name,products.id,proadjust.qtn,proadjust.type,proadjust.des,strftime('%d/%m/%Y',proadjust.date),users.name FROM proadjust INNER JOIN products ON proadjust.pid=products.id LEFT JOIN users ON proadjust.uid=users.id WHERE proadjust.date BETWEEN ? AND ?",(fromd,tod,))
    ...
    def loadData(self):
        ...
        self.sale="0"  
        cur = self.conn.cursor()
        result = cur.execute("SELECT proadjust.id,products.name,products.id,proadjust.qtn,proadjust.type,proadjust.des,strftime('%d/%m/%Y',proadjust.date),users.name FROM proadjust INNER JOIN products ON proadjust.pid=products.id LEFT JOIN users ON proadjust.uid=users.id ORDER BY proadjust.id DESC")
        self.tableWidget.setRowCount(0)
        for row_number, row_data in enumerate(result):
            self.tableWidget.insertRow(row_number)
            for column_number, data in enumerate(row_data):
    ...
    def addS(self):
        ...
        if(name=="" and id==""):
            ...
            if selectdata=="Product Minus":
                proad = cur.execute("INSERT INTO proadjust(pid,qtn,des,type,uid)VALUES(?,?,?,?,?)",(self.id,qtns,des,selectdata,self.uid,))
                self.conn.commit()
                ...
                if(result):
                    ...
                    cur.execute("INSERT INTO pledger(type,pid,adsujt_id,uid,price,qtn)VALUES(?,?,?,?,?,?)",query3)
                    self.conn.commit()                      
            ...
            if selectdata=="Product Increae":
                ...
                quantity = float(self.quantity)
                qtn = quantity+qtns
                result = cur.execute("UPDATE products SET qtn=? WHERE id=?",(qtn,id))
                self.conn.commit()
                if(result):
                    id = proad.lastrowid
        ...
        if(reply == QMessageBox.Yes):          
            if type=="Product Minus":
                if id=="":
                    ...
                    qtn = float(qtn) 
                    cur.execute("DELETE FROM proadjust WHERE id=?",(id,))
                    self.conn.commit()
                    ...
                    self.conn.commit()
                    if(result):
                        cur.execute("DELETE FROM pledger WHERE adsujt_id=?",(id,))
                        self.conn.commit()                          
